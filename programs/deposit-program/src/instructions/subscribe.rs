...
pub fn subscribe(ctx: Context<Subscribe>, amount: u64, duration: i64) -> Result<()> {
    ...
    msg!(
        "Subscription payment of {} tokens has been sent to the master wallet.",
        amount
    );
    ...
    if ctx.accounts.user_info.subscription_ends_at > current_timestamp {
        ctx.accounts.user_info.subscription_ends_at += duration;
        msg!(
            "Extended subscription until {}.",
            ctx.accounts.user_info.subscription_ends_at
        );
    } else {
        ctx.accounts.user_info.subscription_ends_at = current_timestamp + duration;
        msg!(
            "New subscription active until {}.",
            ctx.accounts.user_info.subscription_ends_at
        );
    }
